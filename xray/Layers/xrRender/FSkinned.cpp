...
void CSkeletonX_ext::_Load_hw(Fvisual& V, void* _verts_)
{
    ...
    //	u32		dwUsage				= /*D3DUSAGE_WRITEONLY |*/ (bSoft?D3DUSAGE_SOFTWAREPROCESSING:0);	// VB may be
    //read by wallmarks code
    switch (RenderMode)
    {
    ...
    case RM_SKINNING_1B: {
        ...
        // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_1W*		dst	= (vertHW_1W*)bytes; vertBoned1W*	src
        // = (vertBoned1W*)_verts_; for (u32 it=0; it<V.vCount; it++)	{ Fvector2	uv; uv.set(src->u,src->v); dst->set
        // (src->P,src->N,src->T,src->B,uv,src->matrix*3); dst++; src++;
        //}
    ...
    case RM_SKINNING_2B: {
        ...
        // R_CHK
        // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0)); R_CHK
        // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_2W* dst		= (vertHW_2W*)bytes; vertBoned2W* src
        // = (vertBoned2W*)_verts_;
    ...
    case RM_SKINNING_3B: {
        ...
        // R_CHK
        // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0)); R_CHK
        // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_3W* dst			= (vertHW_3W*)bytes; vertBoned3W*
        // src		= (vertBoned3W*)_verts_;
    ...
    case RM_SKINNING_4B: {
        ...
        // R_CHK
        // (HW.pDevice->CreateVertexBuffer(V.vCount*vStride,dwUsage,0,D3DPOOL_MANAGED,&V.p_rm_Vertices,0)); R_CHK
        // (V.p_rm_Vertices->Lock(0,0,(void**)&bytes,0)); vertHW_4W* dst			= (vertHW_4W*)bytes; vertBoned4W*
        // src		= (vertBoned4W*)_verts_;
    ...
    const u16* indices, u32 vertex_idx, u32 idx)
{
    ...
    for (u8 i = 0; i < vertex_type::bones_count; ++i)
        if (v.get_bone_id(i) >= Parent->LL_BoneCount())
        {
            ...
            Msg(" iBase: %d, iCount: %d, V->iBase %d, V->iCount %d, V->vBase: %d,  V->vCount  %d, vertex_idx: %d, idx: "
                "%d",

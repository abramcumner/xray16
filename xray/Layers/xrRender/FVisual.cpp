...
void Fvisual::Load(const char* N, IReader* data, u32 dwFlags)
{
    ...
    if (!loaded_v)
    {
        ...
        else
        {
            ...
            // model-wallmarks code BYTE*	bytes		= 0;

            // VERIFY				(NULL==p_rm_Indices);
            // R_CHK
            // (HW.pDevice->CreateIndexBuffer(iCount*2,dwUsage,D3DFMT_INDEX16,D3DPOOL_MANAGED,&p_rm_Indices,0)); R_CHK
            // (p_rm_Indices->Lock(0,0,(void**)&bytes,0)); CopyMemory		(bytes, data->pointer(), iCount*2);

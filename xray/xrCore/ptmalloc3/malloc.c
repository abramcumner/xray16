...
#define XRAY_PLATFORM_WINDOWS_32
...
#if USE_LOCKS && (defined(__GNUC__) && (defined(__i386__) || defined(__x86_64__))) ||                                  \
...
#define CALL_MREMAP(addr, osz, nsz, mv) ((void)(addr), (void)(osz), (void)(nsz), (void)(mv), MFAIL)
...
static FORCEINLINE int pthread_init_lock(MLOCK_T* sl)
{
    ...
    if (pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE))
        return 1;
    if (pthread_mutex_init(&sl->l, &attr))
        return 1;
    ...
    {                                                                                                                  \
        ...
        if (F == B)                                                                                                    \
            clear_smallmap(M, I);                                                                                      \
                     ...
                     (F == smallbin_at(M, I) || ok_address(M, F)) && (B == smallbin_at(M, I) || ok_address(M, B))))    \
        {                                                                                                              \
            F->bk = B;                                                                                                 \
            B->fd = F;                                                                                                 \
        }                                                                                                              \
...
#if !NO_MALLINFO
struct mallinfo dlmallinfo(void) { return internal_mallinfo(gm); }
#endif /* NO_MALLINFO */
...
#ifdef USE_OUT_OF_MEMORY_HANDLER
void const* out_of_memory_handler_parameter(mspace arena) { return ((mstate)(arena))->out_of_memory_parameter; }
#endif // #ifdef USE_OUT_OF_MEMORY_HANDLER
...
#ifndef XRAY_PLATFORM_WINDOWS_32

static const int MultiplyDeBruijnBitPosition[32] = {0, 1, 28, 2, 29, 14, 24, 3, 30, 22, 20, 15, 25, 17, 4, 8, 31, 27,

...
BOOL __stdcall RealAssert(DWORD dwOverrideOpts, LPCSTR szMsg, BOOL bAllowHalts);
...
BOOL __stdcall RealAssert(DWORD dwOverrideOpts, LPCSTR szMsg, BOOL bAllowHalts)
{
    ...
    for (loop = g_HMODVector.begin(); loop != g_HMODVector.end(); loop++)
    {
        if (0 !=
            FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE,
                *loop, dwLastErr, 0, (LPSTR)&szFmtMsg, 0, NULL))
        {
            break;
        }
    }
    ...
    if (NULL == szFmtMsg)
    {
        FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM,
            NULL, dwLastErr, 0, (LPSTR)&szFmtMsg, 0, NULL);
    }
    ...
    pCurrPos += (wsprintfA(szBuff,
        ...
        "%s\n"
        "Last Error (0x%08X) : %s\n",
        szModName, szMsg, dwLastErr, szFmtMsg));
    ...
    {
        DoStackTrace(pCurrPos, sizeof(szBuff) - ((DWORD)pCurrPos - (DWORD)szBuff));
    }
    ...
    {
        DWORD dwWritten;
        WriteFile(g_hAssertFile, szBuff, lstrlenA(szBuff), &dwWritten, NULL);
    }
    ...
    if (DA_SHOWMSGBOX == (DA_SHOWMSGBOX & dwOpts))
    {
        ...
        iRet = MessageBoxA(hWndParent, szBuff, "ASSERTION FAILURE...", uiType);
    }
...
void DiagOutputA(LPCSTR szFmt, ...)
{
    ...
    {
        DWORD dwWritten;
        WriteFile(g_hTraceFile, szOutBuff, lstrlenA(szOutBuff), &dwWritten, NULL);
    }
...
void DiagOutputW(LPCWSTR szFmt, ...)
{
    ...
    {
        DWORD dwWritten;
        WriteFile(g_hTraceFile, szOutBuff, lstrlenW(szOutBuff), &dwWritten, NULL);
    }
...
void __stdcall DiagOutputVB(LPCSTR szMsg) { DiagOutputA(szMsg); }
...
static DWORD __stdcall GetModBase(HANDLE hProcess, DWORD dwAddr)
{
    ...
    else
    {
        ...
        if (0 != VirtualQueryEx(hProcess, (LPCVOID)dwAddr, &stMBI, sizeof(stMBI)))
        {
            ...
            DWORD dwRet =
#endif
                g_cSym.SymLoadModule(hFile, (dwNameLen ? szFile : NULL), NULL, (DWORD)stMBI.AllocationBase, 0);
#ifdef _DEBUG
            if (0 == dwRet)
            {
...
static DWORD ConvertAddress(DWORD dwAddr, LPTSTR szOutBuff)
{
    ...
    if (0 != g_cSym.SymGetSymFromAddr(dwAddr, &dwDisp64, pIHS))
    {
        ...
        {
            pCurrPos += wsprintf(pCurrPos, _T ( "%s + %d bytes" ), pIHS->Name, dwDisp64);
        }
...
extern BOOL __stdcall CH_ReadProcessMemory(
...
void DoStackTrace(LPTSTR szString, DWORD dwSize)
{
    ...
    if (FALSE == g_bSymIsInit)
    {
        ...
        if (FALSE == g_cSym.SymInitialize(hProcess, NULL, FALSE))
        {
#ifdef _DEBUG
            TRACE(
                "DiagAssert : Unable to initialize the "
                "symbol engine!!!\n");
            DebugBreak();
#endif
        }
...
#error( "Unknown machine!" )
        ...
        {
            if (FALSE ==
                StackWalk(dwMachine, hProcess, hThread, &stFrame, &stCtx,

...
BOOL InternalSymGetLineFromAddr(
    IN HANDLE hProcess, IN DWORD_PTR dwAddr, OUT PDWORD pdwDisplacement, OUT PIMAGEHLP_LINE Line);
...
BOOL __stdcall AddCrashHandlerLimitModule(HMODULE hMod)
{
    ...
    HMODULE* phTemp = (HMODULE*)HeapAlloc(
        GetProcessHeap(), HEAP_ZERO_MEMORY | HEAP_GENERATE_EXCEPTIONS, (sizeof(HMODULE) * (g_uiModCount + 1)));
    ASSERT(NULL != phTemp);
    if (NULL == phTemp)
    {
        TRACE0(
            "Serious trouble in the house! - "
            "HeapAlloc failed!!!\n");
        return (FALSE);
    }
...
UINT __stdcall GetLimitModuleCount(void) { return (g_uiModCount); }
        ...
        if (NULL != g_pfnCallBack)
        {
            ...
            else
            {
                HINSTANCE hBaseAddr = (HINSTANCE)SymGetModuleBase(
                    (HANDLE)GetCurrentProcessId(), (DWORD)pExPtrs->ExceptionRecord->ExceptionAddress);
                if (NULL != hBaseAddr)
                {
...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    __try
    {
        ...
        if (NULL != dwTemp)
        {
            iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), (LPCTSTR)dwTemp);
        }
        else
        {
            iCurr += (FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE,
                          GetModuleHandle(_T("NTDLL.DLL")), pExPtrs->ExceptionRecord->ExceptionCode, 0,
                          g_szBuff + iCurr, BUFF_SIZE, 0) *
                sizeof(TCHAR));
        }
        ...
        dwTemp = SymGetModuleBase(GetCurrentProcess(), (DWORD_PTR)pExPtrs->ExceptionRecord->ExceptionAddress);
        ASSERT(NULL != dwTemp);
...
#ifdef _WIN64
        iCurr += wsprintf(g_szBuff + iCurr, _T ( " at %016X" ), pExPtrs->ExceptionRecord->ExceptionAddress);
#else
        iCurr += wsprintf(g_szBuff + iCurr, _T ( " at %04X:%08X" ), pExPtrs->ContextRecord->SegCs,
        ...
        if (TRUE ==
            SymGetSymFromAddr(
                GetCurrentProcess(), (DWORD_PTR)pExPtrs->ExceptionRecord->ExceptionAddress, &dwDisp64, pSym))
        {
            ...
            if ((int)dwTemp > ((BUFF_SIZE - iCurr) - (MAX_SYM_SIZE + 50)))
            {
            ...
            else
            {
                if (dwDisp64 > 0)
                {
                    iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s()+%04d byte(s)" ), pSym->Name, dwDisp64);
                }
                else
                {
        ...
        if (TRUE ==
            InternalSymGetLineFromAddr(
                GetCurrentProcess(), (DWORD_PTR)pExPtrs->ExceptionRecord->ExceptionAddress, &dwDisp, &g_stLine))
        {
            iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
            ...
            if ((int)dwTemp > (BUFF_SIZE - iCurr - MAX_PATH - 50))
            {
            ...
            else
            {
                if (dwDisp > 0)
                {
                    iCurr += wsprintf(g_szBuff + iCurr, _T("%s, line %04d+%04d byte(s)"), g_stLine.FileName,
                        g_stLine.LineNumber, dwDisp);
                }
                else
                {
                    iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s, line %04d"), g_stLine.FileName, g_stLine.LineNumber);
                }
            }
        }
        szRet = g_szBuff;
...
#error( "Unknown machine!" )
        ...
        BOOL bSWRet = StackWalk(CH_MACHINE, GetCurrentProcess(), GetCurrentThread(), &g_stFrame, pExPtrs->ContextRecord,
            (PREAD_PROCESS_MEMORY_ROUTINE)CH_ReadProcessMemory, SymFunctionTableAccess, SymGetModuleBase, NULL);
        ...
        dwModBase = SymGetModuleBase(GetCurrentProcess(), g_stFrame.AddrPC.Offset);
        int iCurr = 0;

        // At a minimum, put in the address.
#ifdef _WIN64
        iCurr += wsprintf(g_szBuff + iCurr, _T ( "0x%016X" ), g_stFrame.AddrPC.Offset);
#else
        ...
        if (GSTSO_MODULE == (dwOpts & GSTSO_MODULE))
        {
            ...
            ASSERT(iCurr < (BUFF_SIZE - MAX_PATH));
            iCurr +=
                BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)dwModBase, g_szBuff + iCurr, BUFF_SIZE - iCurr);
        }
        ...
        if (GSTSO_SYMBOL == (dwOpts & GSTSO_SYMBOL))
        {
            ...
            if (TRUE == SymGetSymFromAddr(GetCurrentProcess(), g_stFrame.AddrPC.Offset, &dwDisp64, pSym))
            {
                iCurr += wsprintf(g_szBuff + iCurr, _T ( ", " ));
                ...
                else
                {
                    if (dwDisp64 > 0)
                    {
                        iCurr += wsprintf(g_szBuff + iCurr, _T("%s + %d byte(s)"), pSym->Name, dwDisp64);
                    }
                    else
                    {
                        iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), pSym->Name);
        ...
        if (GSTSO_SRCLINE == (dwOpts & GSTSO_SRCLINE))
        {
            ...
            if (TRUE == InternalSymGetLineFromAddr(GetCurrentProcess(), g_stFrame.AddrPC.Offset, &dwDisp, &g_stLine))
            {
                ...
                else
                {
                    if (dwDisp > 0)
                    {
                        iCurr += wsprintf(g_szBuff + iCurr, _T("%s, %d + %d byte(s)"), g_stLine.FileName,
                            g_stLine.LineNumber, dwDisp);
                    }
                    else
                    {
...
LPCTSTR __stdcall GetRegisterString(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    wsprintf(g_szBuff,
        _T("EAX=%08X  EBX=%08X  ECX=%08X  EDX=%08X  ESI=%08X\n"
           ...
           "FS=%04X  GS=%04X"),
        pExPtrs->ContextRecord->Eax, pExPtrs->ContextRecord->Ebx, pExPtrs->ContextRecord->Ecx,
        pExPtrs->ContextRecord->Edx, pExPtrs->ContextRecord->Esi, pExPtrs->ContextRecord->Edi,
        pExPtrs->ContextRecord->Ebp, pExPtrs->ContextRecord->Esp, pExPtrs->ContextRecord->Eip,
        pExPtrs->ContextRecord->EFlags, pExPtrs->ContextRecord->SegCs, pExPtrs->ContextRecord->SegDs,
        pExPtrs->ContextRecord->SegSs, pExPtrs->ContextRecord->SegEs, pExPtrs->ContextRecord->SegFs,
...
LPCTSTR ConvertSimpleException(DWORD dwExcept)
{
    switch (dwExcept)
    {
    case EXCEPTION_ACCESS_VIOLATION: return (_T ( "EXCEPTION_ACCESS_VIOLATION" )); break;

    case EXCEPTION_DATATYPE_MISALIGNMENT: return (_T ( "EXCEPTION_DATATYPE_MISALIGNMENT" )); break;

    case EXCEPTION_BREAKPOINT: return (_T ( "EXCEPTION_BREAKPOINT" )); break;

    case EXCEPTION_SINGLE_STEP: return (_T ( "EXCEPTION_SINGLE_STEP" )); break;

    case EXCEPTION_ARRAY_BOUNDS_EXCEEDED: return (_T ( "EXCEPTION_ARRAY_BOUNDS_EXCEEDED" )); break;

    case EXCEPTION_FLT_DENORMAL_OPERAND: return (_T ( "EXCEPTION_FLT_DENORMAL_OPERAND" )); break;

    case EXCEPTION_FLT_DIVIDE_BY_ZERO: return (_T ( "EXCEPTION_FLT_DIVIDE_BY_ZERO" )); break;

    case EXCEPTION_FLT_INEXACT_RESULT: return (_T ( "EXCEPTION_FLT_INEXACT_RESULT" )); break;

    case EXCEPTION_FLT_INVALID_OPERATION: return (_T ( "EXCEPTION_FLT_INVALID_OPERATION" )); break;

    case EXCEPTION_FLT_OVERFLOW: return (_T ( "EXCEPTION_FLT_OVERFLOW" )); break;

    case EXCEPTION_FLT_STACK_CHECK: return (_T ( "EXCEPTION_FLT_STACK_CHECK" )); break;

    case EXCEPTION_FLT_UNDERFLOW: return (_T ( "EXCEPTION_FLT_UNDERFLOW" )); break;

    case EXCEPTION_INT_DIVIDE_BY_ZERO: return (_T ( "EXCEPTION_INT_DIVIDE_BY_ZERO" )); break;

    case EXCEPTION_INT_OVERFLOW: return (_T ( "EXCEPTION_INT_OVERFLOW" )); break;

    case EXCEPTION_PRIV_INSTRUCTION: return (_T ( "EXCEPTION_PRIV_INSTRUCTION" )); break;

    case EXCEPTION_IN_PAGE_ERROR: return (_T ( "EXCEPTION_IN_PAGE_ERROR" )); break;

    case EXCEPTION_ILLEGAL_INSTRUCTION: return (_T ( "EXCEPTION_ILLEGAL_INSTRUCTION" )); break;

    case EXCEPTION_NONCONTINUABLE_EXCEPTION: return (_T ( "EXCEPTION_NONCONTINUABLE_EXCEPTION" )); break;

    case EXCEPTION_STACK_OVERFLOW: return (_T ( "EXCEPTION_STACK_OVERFLOW" )); break;

    case EXCEPTION_INVALID_DISPOSITION: return (_T ( "EXCEPTION_INVALID_DISPOSITION" )); break;

    case EXCEPTION_GUARD_PAGE: return (_T ( "EXCEPTION_GUARD_PAGE" )); break;

    case EXCEPTION_INVALID_HANDLE: return (_T ( "EXCEPTION_INVALID_HANDLE" )); break;
    ...
    IN HANDLE hProcess, IN DWORD_PTR dwAddr, OUT PDWORD pdwDisplacement, OUT PIMAGEHLP_LINE Line)
{
    ...
    DWORD dwTempDis = 0;
    while (FALSE == SymGetLineFromAddr(hProcess, dwAddr - dwTempDis, pdwDisplacement, Line))
    {
        dwTempDis += 1;
        if (100 == dwTempDis)
        {
...
void InitSymEng(void)
{
    if (FALSE == g_bSymEngInit)
    {
        ...
        // I'm on.
        VERIFY(BSUSymInitialize(GetCurrentProcessId(), GetCurrentProcess(), g_application_path, TRUE));
        g_bSymEngInit = TRUE;
    }
}

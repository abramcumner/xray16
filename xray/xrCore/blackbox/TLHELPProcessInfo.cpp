...
static BOOL InitTOOLHELP32(void)
{
    ...
    if (NULL != hKernel)
    {
        ...
        bRet = g_pModule32First && g_pModule32Next && g_pProcess32First && g_pProcess32Next && g_pThread32First &&
            g_pThread32Next && g_pCreateToolhelp32Snapshot;
    }
    else
    {
    ...
    if (TRUE == g_pModule32First(hModSnap, &stME32))
    {
        do
        {
            ...
            if (0 != uiCount)
            {
                ...
                if ((TRUE == bBuffToSmall) || (*pdwRealCount == uiCount))
                {
                    bBuffToSmall = TRUE;
                    break;
                }
                ...
                {
                    // Copy this value in.
                    paModArray[*pdwRealCount] = (HINSTANCE)stME32.modBaseAddr;
                }
            }
...
DWORD __stdcall TLHELPGetModuleFileNameEx(
    DWORD dwPID, HANDLE /*hProcess*/, HMODULE hModule, LPTSTR szFilename, DWORD nSize)
{
    if (FALSE == InitTOOLHELP32())
    {
        ASSERT(!"InitTOOLHELP32 failed");

...
DWORD __stdcall BSUSymInitialize(DWORD dwPID, HANDLE hProcess, PSTR UserSearchPath, BOOL fInvadeProcess)
{
    ...
    else
    {
        ...
        BOOL bSymInit = ::SymInitialize(hProcess, UserSearchPath, fInvadeProcess);
        ...
        if (FALSE == GetLoadedModules(dwPID, 0, NULL, &dwCount))
        {
        ...
        for (UINT uiCurr = 0; uiCurr < dwCount; uiCurr++)
        {
            ...
            if (FALSE == GetModuleFileName(paMods[uiCurr], szModName, sizeof(szModName)))
            {
            ...
            // ends.
            HANDLE hFile = CreateFile(szModName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
...
DWORD __stdcall BSUGetModuleFileNameEx(DWORD dwPID, HANDLE hProcess, HMODULE hModule, LPTSTR szFilename, DWORD nSize)
{
    ...
    return (TLHELPGetModuleFileNameEx(dwPID, hProcess, hModule, szFilename, nSize));
}
...
std::string dump_string(LPCSTR name, const Fvector& v) { return make_string("%s : (%f,%f,%f) ", name, v.x, v.y, v.z); }

void dump(LPCSTR name, const Fvector& v) { Msg("%s", dump_string(name, v).c_str()); }

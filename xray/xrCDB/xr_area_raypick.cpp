...
BOOL CObjectSpace::RayPick(
    const Fvector& start, const Fvector& dir, float range, rq_target tgt, rq_result& R, CObject* ignore_object)
{
    Lock.Enter();
        ...
        if ((R.tgt & d_mask) && sd_test.is_any(d_mask) && (next_test & d_mask))
        {
            ...
            if (d_rd.range > EPS)
            {
                ...
                for (u32 o_it = 0; o_it < r_spatial.size(); o_it++)
                {
                    ...
                    if (!((0 == r_temp.r_count()) ||
                            (r_temp.r_count() &&
                                (fis_zero(r_temp.r_begin()->range, EPS) || (r_temp.r_begin()->range >= 0.f)))))

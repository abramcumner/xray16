...
static unsigned char Fields_Of_Interest[] = {HOSTNAME_KEY, HOSTPORT_KEY, NUMPLAYERS_KEY, MAXPLAYERS_KEY, MAPNAME_KEY,
    GAMETYPE_KEY, GAMEVER_KEY, PASSWORD_KEY, G_USER_PASSWORD_KEY, DEDICATED_KEY, GAMETYPE_NAME_KEY};

XRGAMESPY_API int xrGS_GetQueryVersion() { return QVERSION_QR2; }

// XRGAMESPY_API ServerBrowser xrGS_ServerBrowserNew(const gsi_char *queryForGamename, const gsi_char
// *queryFromGamename, const gsi_char *queryFromKey, int queryFromVersion, int maxConcUpdates, int queryVersion, SBBool
// bLAN_Only, ServerBrowserCallback callback, void *instance)
XRGAMESPY_API ServerBrowser xrGS_ServerBrowserNewA(SBBool bLAN_Only, ServerBrowserCallback callback, void* instance)
{
    //	return ServerBrowserNew(queryForGamename, queryFromGamename, queryFromKey, queryFromVersion, maxConcUpdates,
    //queryVersion, bLAN_Only, callback, instance);
    char SecretKey[16];
    FillSecretKey(SecretKey);
    return ServerBrowserNewA(GAMESPY_GAMENAME, GAMESPY_GAMENAME, SecretKey, 0, GAMESPY_BROWSER_MAX_UPDATES,
        xrGS_GetQueryVersion(), bLAN_Only, callback, instance);
}

XRGAMESPY_API void xrGS_ServerBrowserFree(ServerBrowser sb) { ServerBrowserFree(sb); };

XRGAMESPY_API void xrGS_ServerBrowserClear(ServerBrowser sb) { ServerBrowserClear(sb); };

XRGAMESPY_API SBError xrGS_ServerBrowserThink(ServerBrowser sb) { return ServerBrowserThink(sb); };

XRGAMESPY_API SBState xrGS_ServerBrowserState(ServerBrowser sb) { return ServerBrowserState(sb); };

XRGAMESPY_API void xrGS_ServerBrowserHalt(ServerBrowser sb) { ServerBrowserHalt(sb); };

// XRGAMESPY_API SBError xrGS_ServerBrowserUpdate (ServerBrowser sb, SBBool async, SBBool disconnectOnComplete, const
// unsigned char *basicFields, int numBasicFields, const gsi_char *serverFilter)
XRGAMESPY_API SBError xrGS_ServerBrowserUpdateA(
    ServerBrowser sb, SBBool async, SBBool disconnectOnComplete, const gsi_char* serverFilter)
{
...
XRGAMESPY_API void xrGS_ServerBrowserSortA(
    ServerBrowser sb, SBBool ascending, const char* sortkey, SBCompareMode comparemode)
{
    ServerBrowserSortA(sb, ascending, sortkey, comparemode);
};

// XRGAMESPY_API SBError xrGS_ServerBrowserLANUpdate(ServerBrowser sb, SBBool async, unsigned short startSearchPort,
...
XRGAMESPY_API int xrGS_ServerBrowserCount(ServerBrowser sb) { return ServerBrowserCount(sb); };
...
XRGAMESPY_API char* xrGS_SBServerGetPublicAddress(SBServer server) { return SBServerGetPublicAddress(server); };
XRGAMESPY_API unsigned short xrGS_SBServerGetPublicQueryPort(SBServer server)
{
    return SBServerGetPublicQueryPort(server);
};
XRGAMESPY_API const gsi_char* xrGS_SBServerGetStringValueA(
    SBServer server, const gsi_char* keyname, const gsi_char* def)
{
...
XRGAMESPY_API int xrGS_SBServerGetPing(SBServer server) { return SBServerGetPing(server); };

XRGAMESPY_API SBError xrGS_ServerBrowserAuxUpdateServer(
...
XRGAMESPY_API const gsi_char* xrGS_SBServerGetPlayerStringValueA(
...
XRGAMESPY_API double xrGS_SBServerGetPlayerFloatValueA(
    SBServer server, int playernum, const gsi_char* key, double fdefault)
{
    return SBServerGetPlayerFloatValueA(server, playernum, key, fdefault);
}

XRGAMESPY_API const gsi_char* xrGS_SBServerGetTeamStringValueA(
    SBServer server, int teamnum, const gsi_char* key, const gsi_char* sdefault)
{
...
XRGAMESPY_API SBBool xrGS_SBServerGetConnectionInfo(
    ServerBrowser sb, SBServer server, gsi_u16 PortToConnectTo, char* ipstring)
{
    return SBServerGetConnectionInfo(sb, server, PortToConnectTo, ipstring);
};

XRGAMESPY_API SBBool xrGS_SBServerDirectConnect(SBServer server) { return SBServerDirectConnect(server); };

XRGAMESPY_API SBBool xrGS_SBServerHasFullKeys(SBServer server) { return SBServerHasFullKeys(server); }

...
extern int _NetworkAdHocSocketSendTo(
    int adhoc_socket, const void* data, int len, int flags, const char* dest_addr, gsi_u16 dest_port);
...
static void gti2ConnectionFree(void* elem) { gti2ConnectionCleanup(*(GT2Connection*)elem); }
...
GT2Result gti2CreateSocket(GT2Socket* sock, const char* localAddress, int outgoingBufferSize, int incomingBufferSize,
    gt2SocketErrorCallback callback, GTI2ProtocolType type)
{
    SOCKADDR_IN address;
...
void gti2Listen(GT2Socket socket, gt2ConnectAttemptCallback callback) { socket->connectAttemptCallback = callback; }

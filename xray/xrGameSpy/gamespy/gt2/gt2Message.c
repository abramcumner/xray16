...
static int gti2SNDiff(unsigned short SN1, unsigned short SN2) { return (short)(SN1 - SN2); }
...
static GT2Bool gti2HandleClientResponse(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    if (!gti2ConnectAttemptCallback(connection->socket, connection, connection->ip, connection->port, latency,
            message + GTI2_RESPONSE_LEN, len - GTI2_RESPONSE_LEN))
        return GT2False;
...
static GT2Bool gti2BufferIncomingMessage(
    GT2Connection connection, GTI2MessageType type, unsigned short SN, GT2Byte* message, int len, GT2Bool* overflow)
{
...
GT2Bool gti2SendServerChallenge(
    GT2Connection connection, const char response[GTI2_RESPONSE_LEN], const char challenge[GTI2_CHALLENGE_LEN])
{
...
GT2Bool gti2SendClientResponse(
    GT2Connection connection, const char response[GTI2_RESPONSE_LEN], const char* message, int len)
{

...
#ifdef _XBOX
GT2Result gt2CreateVDPSocket(GT2Socket* socket, const char* localAddress, int outgoingBufferSize,
...
GT2Result gt2CreateSocket(GT2Socket* socket, const char* localAddress, int outgoingBufferSize, int incomingBufferSize,
...
GT2Result gt2CreateAdHocSocket(GT2Socket* socket, const char* localAddress, int outgoingBufferSize,
    int incomingBufferSize, gt2SocketErrorCallback callback)
{
...
void gt2Listen(GT2Socket socket, gt2ConnectAttemptCallback callback) { gti2Listen(socket, callback); }
...
GT2Result gt2Connect(GT2Socket socket, GT2Connection* connection, const char* remoteAddress, const GT2Byte* message,
    int len, int timeout, GT2ConnectionCallbacks* callbacks, GT2Bool blocking)
{
...
void gt2Ping(GT2Connection connection) { gti2SendPing(connection); }

void gt2CloseConnection(GT2Connection connection) { gti2CloseConnection(connection, GT2False); }

void gt2CloseConnectionHard(GT2Connection connection) { gti2CloseConnection(connection, GT2True); }
...
void gt2CloseAllConnections(GT2Socket socket) { TableMapSafe(socket->connections, gti2CloseAllConnectionsMap, NULL); }
...
*************************/
GT2MessageID gt2GetLastSentMessageID(GT2Connection connection) { return (GT2MessageID)(connection->serialNumber - 1); }
...
GT2Socket gt2GetConnectionSocket(GT2Connection connection) { return connection->socket; }
...
unsigned int gt2GetRemoteIP(GT2Connection connection) { return connection->ip; }

unsigned short gt2GetRemotePort(GT2Connection connection) { return connection->port; }

unsigned int gt2GetLocalIP(GT2Socket socket) { return socket->ip; }

unsigned short gt2GetLocalPort(GT2Socket socket) { return socket->port; }

int gt2GetIncomingBufferSize(GT2Connection connection) { return connection->incomingBuffer.size; }
...
int gt2GetOutgoingBufferSize(GT2Connection connection) { return connection->outgoingBuffer.size; }
...
SOCKET gt2GetSocketSOCKET(GT2Socket socket) { return socket->socket; }
...
void gt2SetSendDump(GT2Socket socket, gt2DumpCallback callback) { socket->sendDumpCallback = callback; }

void gt2SetReceiveDump(GT2Socket socket, gt2DumpCallback callback) { socket->receiveDumpCallback = callback; }
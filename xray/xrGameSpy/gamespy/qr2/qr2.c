    ...
    qr2_adderrorcallback_t adderror_callback, void* userdata)
{
    ...
    if (ispublic)
    {
        ...
        if (ret == 1)
        {
            gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Network, GSIDebugLevel_Comment, "%s resolved to %s\r\n",
    ...
    qr2_adderrorcallback_t adderror_callback, void* userdata)
{
    ...
    return qr2_init_socketA(qrec, s, boundport, gamename_A, secretkey_A, ispublic, natnegotiate, server_key_callback,
    ...
    qr2_adderrorcallback_t adderror_callback, void* userdata)
{
    ...
    ret = qr2_init_socketA(qrec, hbsock, baseport, gamename, secret_key, ispublic, natnegotiate, server_key_callback,
        player_key_callback, team_key_callback, key_list_callback, playerteam_count_callback, adderror_callback,
        userdata);
    if (qrec == NULL)
        qrec = &current_rec;
    ...
    qr2_adderrorcallback_t adderror_callback, void* userdata)
{
    ...
    return qr2_initA(qrec, (ip != NULL) ? ip_A : NULL, baseport, gamename_A, secretkey_A, ispublic, natnegotiate,
...
static void qr_build_query_reply(qr2_t qrec, qr2_buffer_t buf, int serverkeycount, uchar* serverkeys,
    int playerkeycount, uchar* playerkeys, int teamkeycount, uchar* teamkeys)
{
...
// #define SIMULATE_FIREWALL
    ...
    case PACKET_CHALLENGE:
        ...
        if (len >= (PUBLIC_ADDR_LEN + 3))
        {
            ...
            gsDebugFormat(GSIDebugCat_QR2, GSIDebugType_Misc, GSIDebugLevel_Notice, "Received setting options: %d\r\n",
                qrec->backendoptions);
...
static void send_heartbeat(qr2_t qrec, int statechanged)
{
    ...
    // ret = (int)sendto(qrec->hbsock, buf.buffer, buf.len, 0, (struct sockaddr *)&(qrec->hbaddr), sizeof(struct
    // sockaddr_in));
    sendto(qrec->hbsock, buf.buffer, buf.len, 0, (struct sockaddr*)&(qrec->hbaddr), sizeof(struct sockaddr_in));

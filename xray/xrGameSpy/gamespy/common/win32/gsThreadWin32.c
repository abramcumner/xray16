...
void gsiCloseSemaphore(GSISemaphoreID theSemaphore) { CloseHandle(theSemaphore); }
...
void gsiExitThread(GSIThreadID id) { GSI_UNUSED(id); }

void gsiCleanupThread(GSIThreadID id) { CloseHandle(id); }
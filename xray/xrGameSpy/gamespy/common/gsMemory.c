...
static void* MEM_MANAGER_CALL _gsi_malloc(size_t size) { return malloc(size); }

static void MEM_MANAGER_CALL _gsi_free(void* ptr) { free(ptr); }

static void* MEM_MANAGER_CALL _gsi_realloc(void* ptr, size_t size) { return realloc(ptr, size); }

#if defined(_PS2) || defined(_PSP) || defined(_PS3)
static void* _gsi_memalign(size_t boundary, size_t size) { return memalign(boundary, size); }
#elif defined(_WIN32)
#if (_MSC_VER < 1300)
// extern added for vc6 compatability.
extern void* __cdecl _aligned_malloc(size_t size, int boundary);
#endif
static void* __cdecl _gsi_memalign(size_t boundary, size_t size) { return _aligned_malloc(size, (int)boundary); }
#else
...
void* gsimalloc(size_t size) { return (*memmanagercallbacks.malloc)(size); }
void* gsirealloc(void* ptr, size_t size) { return (*memmanagercallbacks.realloc)(ptr, size); }
...
void* gsimemalign(size_t boundary, size_t size) { return (*memmanagercallbacks.memalign)(boundary, size); }
...
void MEM_CHUNKFree(MEM_CHUNK* _this) { _this->MEM_UsageStat.MemUsed = 0; }

/***************************************/
// returns true if not in use
gsi_bool MEM_CHUNKIsFree(MEM_CHUNK* _this) { return (_this->MEM_UsageStat.MemUsed == 0); }
...
gsi_u32 MEM_CHUNKMemUsedGet(MEM_CHUNK* _this) { return (_this->MEM_UsageStat.MemUsed & ~0xFF) >> 6; }
...
gsi_u32 MEM_CHUNKMemAvailGet(MEM_CHUNK* _this) { return MEM_CHUNKChunkSizeGet(_this) - MEM_CHUNKMemUsedGet(_this); }

char MEM_CHUNKMemTypeGet(MEM_CHUNK* _this) { return _this->MEM_UsageStat.MEM_TypeStruct.MemType; }
...
void* MEM_CHUNKMemPtrGet(MEM_CHUNK* _this) { return (void*)((gsi_uint)_this + sizeof(MEM_CHUNK)); }

/*inline */ MEM_CHUNK* Ptr_To_MEM_CHUNK(void* ptr) { return ((MEM_CHUNK*)ptr) - 1; }
...
MEM_CHUNK* MEM_CHUNK_POOLAllocChunk(MEM_CHUNK_POOL* _this, size_t Size, int Alignment,
...
gsi_bool MEM_CHUNK_POOLIsValid(MEM_CHUNK_POOL* _this) { return _this->HeapSize > 0; }
...
void MEM_CHUNK_POOLFillMemoryTable(
    MEM_CHUNK_POOL* _this, char* Table, const int TableSize, gsi_u32 _HeapStart, gsi_u32 _HeapSize);
...
void MEM_CHUNK_POOLDestroy(MEM_CHUNK_POOL* _this) { memset(_this, 0, sizeof(MEM_CHUNK_POOL)); }
//--------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
{
    ...
    {
        MP_ASSERT(
            (header->next == NULL) || ((gsi_uint)header < (gsi_uint)header->next)) // infinite loop or out of place
        MP_ASSERT(
            (header->prev == NULL) || ((gsi_uint)header->prev < (gsi_uint)header)) // infinite loop or out of place
            ...
            if ((header != _this->HeaderStart) && (header != _this->HeaderEnd))
            {
                ...
                if (_LogUse)
                {
                    gsDebugFormat(GSIDebugCat_App, GSIDebugType_Misc, GSIDebugLevel_Notice,
                        "MemFound ptr:0x%8x  size:%8u %s\n", MEM_CHUNKMemPtrGet(header), MEM_CHUNKMemUsedGet(header),
...
void gsMemMgrContextPush(gsMemMgrContext NewType)
{
    ...
    //	gsDebugFormat(GSIDebugCat_App, GSIDebugType_State, GSIDebugLevel_Comment,"MemProfilerStart:
...
gsMemMgrContext gsMemMgrContextPop()
{
    ...
    //		gsDebugFormat(GSIDebugCat_App, GSIDebugType_State, GSIDebugLevel_Comment,"MemProfilerEnd:
...
gsi_u32 gsMemMgrMemLargestAvailGet(gsMemMgrContext context)
{
    ...
    GS_ASSERT_STR(
        MEM_CHUNK_POOLIsValid(&gChunkPool[context]), "gsMemMgrMemLargestAvailGet: context is invalid mempool");
    MEM_CHUNK_POOLMemStatsGet(&gChunkPool[context], &stats);
...
void gsMemMgrDumpAllocations()
{
    ...
    while (aBlockPtr != NULL)
    {
        ...
        if (gsiMemBlockIsFlagged(aBlockPtr, BlockFlag_Used))
        {
            ...
            {
                gsDebugFormat(GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "\t[0x%08x] Size: %d\r\n",
    ...
    gsDebugFormat(GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "\t--%d peak memory usage\r\n",
        gMemoryMgr->mPeakMemoryUsage);

    gsDebugFormat(GSIDebugCat_App, GSIDebugType_Memory, GSIDebugLevel_Comment, "Memory dump complete. (%d ms)\r\n",
...
int Random(int x) { return Util_RandInt(0, x); }
//--------------------------------------------------------------------------

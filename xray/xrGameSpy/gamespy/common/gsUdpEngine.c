...
void gsUdpConnectedRoutingCB(GT2Connection theConnection, GT2Result theResult, GT2Byte* theMessage, int theMessageLen)
{
    ...
    switch (theResult)
    {
    case GT2NegotiationError: aCode = GS_UDP_REMOTE_ERROR; break;
    case GT2Rejected: aCode = GS_UDP_REJECTED; break;
    case GT2TimedOut: aCode = GS_UDP_TIMED_OUT; break;
    case GT2Success: aCode = GS_UDP_NO_ERROR; break;
    default: aCode = GS_UDP_UNKNOWN_ERROR; break;
    }
    ...
    for (aIndex = 0; aIndex < len; aIndex++)
    {
        ...
        if (aRemotePeerIdx != NOT_FOUND)
        {
            if (aTempHandler->mConnected)
            {
                ...
                aTempHandler->mConnected(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), aCode,
                    theResult == GT2Rejected ? gsi_true : gsi_false, aTempHandler->mUserData);
            }
    ...
    if (aUdp->mAppPendingConnections > 0)
    {
        if (aUdp->mAppConnected)
        {
            ...
            aUdp->mAppConnected(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), aCode,
                theResult == GT2Rejected ? gsi_true : gsi_false, aUdp->mAppUserData);
        }
...
void gsUdpPingRoutingCB(GT2Connection theConnection, int theLatency)
{
    ...
    for (index = 0; index < len; index++)
    {
        ...
        if (aHandler->mPingReply)
        {
            ...
            aHandler->mPingReply(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection),
                (unsigned int)theLatency, aHandler->mUserData);
            return;
        }
    }
    ...
    if (aUdp->mAppPingReply)
    {
        ...
        aUdp->mAppPingReply(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), (unsigned int)theLatency,
            aUdp->mAppUserData);
    }
    GSI_UNUSED(anAddr);
}
...
void gsUdpReceivedRoutingCB(GT2Connection theConnection, GT2Byte* theMessage, int theMessageLen, GT2Bool reliable)
{
        ...
        if (index != NOT_FOUND)
        {
            ...
            if (aHandlerFound->mReceived)
            {
                ...
                aHandlerFound->mReceived(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection),
                    theMessage + GS_UDP_MSG_HEADER_LEN, (unsigned int)(theMessageLen - GS_UDP_MSG_HEADER_LEN), reliable,
    ...
    {
        gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network, GSIDebugLevel_Comment, "[Udp Engine] Passed to app\n");
        aUdp->mAppRecvData(gt2GetRemoteIP(theConnection), gt2GetRemotePort(theConnection), theMessage,
            (unsigned int)theMessageLen, reliable, aUdp->mAppUserData);
    }
    GSI_UNUSED(anAddr);
}
...
GT2Bool gsUdpUnrecognizedMsgCB(
    GT2Socket theSocket, unsigned int theIp, unsigned short thePort, GT2Byte* theMessage, int theMsgLen)
{
    GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
    char anAddr[GS_IP_ADDR_AND_PORT];
...
void gsUdpConnAttemptCB(GT2Socket socket, GT2Connection connection, unsigned int ip, unsigned short port, int latency,
...
// functions below.
GSUdpErrorCode gsUdpEngineInitialize(unsigned short thePort, int theIncomingBufSize, int theOutgoingBufSize,
    gsUdpErrorCallback theAppNetworkError, gsUdpConnConnectedCallback theAppConnected,
    gsUdpConnClosedCallback theAppClosed, gsUdpConnPingCallback theAppPing, gsUdpConnReceivedDataCallback theAppReceive,
    gsUdpUnknownMsgCallback theAppUnownMsg, gsUdpAppConnectAttemptCallback theAppConnectAttempt, void* theAppUserData)
{
...
// IP and port.
GSUdpErrorCode gsUdpEngineStartTalkingToPeer(
    unsigned int theIp, unsigned short thePort, char theInitMsg[GS_UDP_MSG_HEADER_LEN], int timeOut)
{
    char anAddr[GS_IP_ADDR_AND_PORT];
    GSUdpRemotePeer aRemotePeer;
    GSUdpMsgHandler aHandler;
    ...
    else
    {
        ...
        gt2Connect(aUdp->mSocket, &aRemotePeer.mConnection, anAddr, (unsigned char*)theInitMsg, GS_UDP_MSG_HEADER_LEN,
            timeOut, &aCallbacks, GT2False);
...
// freeSpace = 1460 - 16 - 7
GSUdpErrorCode gsUdpEngineSendMessage(unsigned int theIp, unsigned short thePort, char theHeader[GS_UDP_MSG_HEADER_LEN],
    unsigned char* theMsg, unsigned int theMsgLen, gsi_bool theReliable)
{
    GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();
...
GSUdpErrorCode gsUdpEngineAddMsgHandler(char theInitMsg[GS_UDP_MSG_HEADER_LEN], char theHeader[GS_UDP_MSG_HEADER_LEN],
    gsUdpErrorCallback theMsgHandlerError, gsUdpConnConnectedCallback theMsgHandlerConnected,
    gsUdpConnClosedCallback theMsgHandlerClosed, gsUdpConnPingCallback theMsgHandlerPing,
    gsUdpConnReceivedDataCallback theMsgHandlerRecv, void* theUserData)
{
    GSUdpEngineObject* aUdp = gsUdpEngineGetEngine();

...
const char* GS_XML_SOAP_DEFAULT_NAMESPACES[GS_XML_SOAP_DEFAULT_NAMESPACE_COUNT] = {
    "SOAP-ENV=\"http://schemas.xmlsoap.org/soap/envelope/\"", "SOAP-ENC=\"http://schemas.xmlsoap.org/soap/encoding/\"",
...
static gsi_bool gsiXmlUtilParseName(
    GSIXmlStreamReader* stream, const char* buffer, int len, int* pos, GSIXmlString* strOut);
static gsi_bool gsiXmlUtilParseString(
    GSIXmlStreamReader* stream, char* buffer, int len, int* pos, GSIXmlString* strOut);
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_bool gsXmlWriteStringElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const char* value)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
    int i = 0;
    int len = 0;
...
gsi_bool gsXmlWriteAsciiStringElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const gsi_char* value)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
    int i = 0;
    int len = 0;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_bool gsXmlWriteUnicodeStringElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const unsigned short* value)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
    int i = 0;
    int len = 0;
...
gsi_bool gsXmlWriteHexBinaryElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const gsi_u8* data, int len)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
    int pos = 0;
    int temp = 0;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_bool gsXmlWriteBase64BinaryElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const gsi_u8* data, int len)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
...
gsi_bool gsXmlWriteDateTimeElement(GSXmlStreamWriter stream, const char* namespaceName, const char* tag, time_t value)
{
    ...
    sprintf(timeString, "%d-%02d-%02dT%02d:%02d:%02dZ", timePtr->tm_year + 1900, timePtr->tm_mon + 1, timePtr->tm_mday,
...
gsi_bool gsXmlWriteLargeIntElement(
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const struct gsLargeInt_s* lint)
{
    GSIXmlStreamWriter* writer = (GSIXmlStreamWriter*)stream;
    ...
    for (i=(reader->mValueReadIndex+1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex == reader->mElemReadIndex)
        {
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName)))
            {
                ...
                *lenOut = UTF8ToUCS2StringLen((const char *)searchValueElem->mValue.mData, (unsigned short *) *valueOut,
...
gsi_bool gsXmlReadChildAsUnicodeStringNT(
    GSXmlStreamReader stream, const char* matchtag, gsi_char valueOut[], int maxLen)
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
gsi_bool gsXmlReadChildAsHexBinary(
    GSXmlStreamReader stream, const char* matchtag, gsi_u8 valueOut[], int maxLen, int* lenOut)
{
    GSIXmlStreamReader* reader = (GSIXmlStreamReader*)stream;
    GSIXmlElement* searchValueElem = NULL;
    int i = 0;
    ...
    for (i = (reader->mValueReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex == reader->mElemReadIndex)
        {
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName)))
            {
                ...
                if (bytesleft == 1)
                {
                    sscanf((char*)(&searchValueElem->mValue.mData[readpos]), "%01x",
                        &temp); // sscanf requires a 4 byte dest
                    valueOut[writepos] = (gsi_u8)temp; // then we convert to byte, to ensure correct byte order
                    readpos += 1;
                    writepos += 1;
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName)))
            {
                if (valueOut)
                {
                    ...
                    if (searchValueElem->mValue.mData)
                        B64Decode((char*)searchValueElem->mValue.mData, (char*)valueOut, searchValueElem->mValue.mLen,
                            lenOut, GS_XML_BASE64_ENCODING_TYPE);
                    else
                        *lenOut = 0;
                }
                else
    ...
    for (i = (reader->mValueReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex == reader->mElemReadIndex)
        {
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName)))
            {
                ...
                sscanf((const char*)searchValueElem->mValue.mData, "%i-%02d-%02dT%02d:%02d:%02d", &timePtr.tm_year,
                    &timePtr.tm_mon, &timePtr.tm_mday, &timePtr.tm_hour, &timePtr.tm_min, &timePtr.tm_sec);

...
static GPResult gpiStartConnect(GPConnection* connection, GPIOperation* operation)
{
    ...
    anError = gsUdpEngineAddMsgHandler(iconnection->mHeader, iconnection->mHeader, NULL, gpiPeerAcceptedCallback,
        gpiPeerLeftCallback, gpiPeerPingReplyCallback, gpiPeerMessageCallback, connection);
    if (anError != GS_UDP_NO_ERROR)
    {
    ...
    if (iconnection->firewall)
    {
        ...
        if (rcode == 0)
            CallbackFatalError(connection, GP_NETWORK_ERROR, GP_NETWORK, "There was an error making a socket
        non-blocking.");
        // Bind the socket.
        ///////////////////
        ...
        if (gsiSocketIsError(rcode))
            CallbackFatalError(connection, GP_NETWORK_ERROR, GP_NETWORK, "There was an error getting a socket's
        addres."); iconnection->peerPort = address.sin_port;
        */
...
GPResult gpiConnect(GPConnection* connection, const char nick[GP_NICK_LEN], const char uniquenick[GP_UNIQUENICK_LEN],
    const char email[GP_EMAIL_LEN], const char password[GP_PASSWORD_LEN], const char authtoken[GP_AUTHTOKEN_LEN],
    const char partnerchallenge[GP_PARTNERCHALLENGE_LEN], const char cdkey[GP_CDKEY_LEN], GPEnum firewall,
    GPIBool newuser, GPEnum blocking, GPCallback callback, void* param)
{
    GPIConnectData* data;
    ...
    switch (firewall)
    {
    case GP_FIREWALL: iconnection->firewall = GPITrue; break;
    case GP_NO_FIREWALL: iconnection->firewall = GPIFalse; break;
    default: Error(connection, GP_PARAMETER_ERROR, "Invalid firewall.");
    }
#endif
...
static GPResult gpiSendLogin(GPConnection* connection, GPIConnectData* data)
{
    ...
    //////////////////////////
    sprintf(buffer, "%s%s%s%s%s%s", data->passwordHash, "                                                ", user,

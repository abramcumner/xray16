...
GPResult gpiSyncNpBuddies(GPConnection* connection)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        gpiProfileSearchUniquenick(connection, npId.handle.data, &iconnection->namespaceID, 1, GP_NON_BLOCKING,
            (GPCallback)gpiSyncNpBuddiesCallback, NULL);
    }
...
GPResult gpiSyncNpBlockList(GPConnection* connection)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        gpiProfileSearchUniquenick(connection, npId.handle.data, &iconnection->namespaceID, 1, GP_NON_BLOCKING,
            (GPCallback)gpiSyncNpBlockListCallback, NULL);
    }

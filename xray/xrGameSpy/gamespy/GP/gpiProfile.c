...
static GPResult gpiReadDiskProfile(GPConnection* connection, GPIBool* failedOut)
{
    ...
    gsDebugFormat(GSIDebugCat_GP, GSIDebugType_File, GSIDebugLevel_Comment, "Reading profile %d from disk cache:\n",
        profile.profileId);
...
GPResult gpiNewProfile(
    GPConnection* connection, const char nick[31], GPEnum replace, GPEnum blocking, GPCallback callback, void* param)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
...
GPResult gpiFindProfileByUser(
    GPConnection* connection, char nick[GP_NICK_LEN], char email[GP_EMAIL_LEN], GPIProfile** profile)
{

...
GPResult gpiProcessConnectionManager(GPConnection* connection)
{
    ...
    do
    {
        ...
        while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
        {
            ...
            gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "CMD: %s\n",
...
GPResult gpiEnable(GPConnection* connection, GPEnum state)
{
    ...
    switch (state)
    {
    case GP_INFO_CACHING: iconnection->infoCaching = GPITrue; break;

    case GP_SIMULATION: iconnection->simulation = GPITrue; break;

    case GP_INFO_CACHING_BUDDY_AND_BLOCK_ONLY: iconnection->infoCachingBuddyAndBlockOnly = GPITrue; break;

    default: Error(connection, GP_PARAMETER_ERROR, "Invalid state.");
    }
...
void gpiReport(GPConnection* connection, void (*report)(const char* output))
{
    ...
    if (nProfiles)
    {
        ...
        report(buf);
        sprintf(buf, "BuddyStatus: %d (%d%%) %d bytes (%d avg)", nBuddyStatus, nBuddyStatus * 100 / nProfiles,
            nBuddyMemory, nBuddyMemory / max(nBuddyStatus, 1));
        report(buf);
        sprintf(buf, "InfoCache: %d (%d%%) %d bytes (%d avg)", nInfoCache, nInfoCache * 100 / nProfiles, nInfoMemory,

...
// gpConnect
////////////
GPResult gpConnect(GPConnection* connection, const gsi_char nick[GP_NICK_LEN], const gsi_char email[GP_EMAIL_LEN],
    const gsi_char password[GP_PASSWORD_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback, void* param);
...
GPResult gpConnectNewUser(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
    ...
    const gsi_char password[GP_PASSWORD_LEN], const gsi_char cdkey[GP_CDKEY_LEN], GPEnum firewall, GPEnum blocking,
    GPCallback callback, void* param);

// gpConnectUniqueNick
//////////////////////
GPResult gpConnectUniqueNick(GPConnection* connection, const gsi_char uniquenick[GP_UNIQUENICK_LEN],
    const gsi_char password[GP_PASSWORD_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback, void* param);

// gpConnectPreAuthenticated
////////////////////////////
GPResult gpConnectPreAuthenticated(GPConnection* connection, const gsi_char authtoken[GP_AUTHTOKEN_LEN],
    const gsi_char partnerchallenge[GP_PARTNERCHALLENGE_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback,
    void* param);
...
// gpCheckUser
//////////////
GPResult gpCheckUser(GPConnection* connection, const gsi_char nick[GP_NICK_LEN], const gsi_char email[GP_EMAIL_LEN],
    const gsi_char password[GP_PASSWORD_LEN], GPEnum blocking, GPCallback callback, void* param);

// gpNewUser
////////////
GPResult gpNewUser(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
    const gsi_char uniquenick[GP_UNIQUENICK_LEN], const gsi_char email[GP_EMAIL_LEN],
    const gsi_char password[GP_PASSWORD_LEN], const gsi_char cdkey[GP_CDKEY_LEN], GPEnum blocking, GPCallback callback,
    void* param);

// gpSuggestUniqueNick
//////////////////////
GPResult gpSuggestUniqueNick(GPConnection* connection, const gsi_char desirednick[GP_UNIQUENICK_LEN], GPEnum blocking,
    GPCallback callback, void* param);

// gpRegisterUniqueNick
///////////////////////
GPResult gpRegisterUniqueNick(GPConnection* connection, const gsi_char uniquenick[GP_UNIQUENICK_LEN],
    const gsi_char cdkey[GP_CDKEY_LEN], GPEnum blocking, GPCallback callback, void* param);
...
// gpNewProfile
///////////////
GPResult gpNewProfile(GPConnection* connection, const gsi_char nick[GP_NICK_LEN], GPEnum replace, GPEnum blocking,
    GPCallback callback, void* param);
...
// gpProfileSearch
//////////////////
GPResult gpProfileSearch(GPConnection* connection, const gsi_char nick[GP_NICK_LEN],
    const gsi_char uniquenick[GP_UNIQUENICK_LEN], const gsi_char email[GP_EMAIL_LEN],
    const gsi_char firstname[GP_FIRSTNAME_LEN], const gsi_char lastname[GP_LASTNAME_LEN], int icquin, GPEnum blocking,
    GPCallback callback, void* param);

// gpProfileSearchUniquenick
////////////////////////////
GPResult gpProfileSearchUniquenick(GPConnection* connection, const gsi_char uniquenick[GP_UNIQUENICK_LEN],
...
//////////////
#ifndef GP_NEW_STATUS_INFO
GPResult gpSetStatus(GPConnection* connection, GPEnum status, const gsi_char statusString[GP_STATUS_STRING_LEN],
    const gsi_char locationString[GP_LOCATION_STRING_LEN]);
#endif

#ifdef GP_NEW_STATUS_INFO
GPResult gpSetStatusInfo(GPConnection* connection, GPEnum statusState, unsigned int hostIp, unsigned int hostPrivateIp,
    unsigned short queryPort, unsigned short hostPort, unsigned int sessionFlags, const gsi_char* richStatus,
    int richStatusLen, const gsi_char* gameType, int gameTypeLen, const gsi_char* gameVariant, int gameVariantLen,
    const gsi_char* gameMapName, int gameMapNameLen);
...
GPResult gpIsValidEmail(
    GPConnection* connection, const gsi_char email[GP_EMAIL_LEN], GPEnum blocking, GPCallback callback, void* param);

// gpGetUserNicks
/////////////////
GPResult gpGetUserNicks(GPConnection* connection, const gsi_char email[GP_EMAIL_LEN],
    const gsi_char password[GP_PASSWORD_LEN], GPEnum blocking, GPCallback callback, void* param);
...
typedef void (*gpSendFilesCallback)(
    GPConnection* connection, int index, const gsi_char** path, const gsi_char** name, void* param);

GPResult gpSendFiles(GPConnection* connection, GPTransfer* transfer, GPProfile profile, const gsi_char* message,

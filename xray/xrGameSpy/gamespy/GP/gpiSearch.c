...
static GPResult gpiStartSearch(
    GPConnection* connection, GPISearchData* data, GPEnum blocking, GPCallback callback, void* param)
{
...
GPResult gpiProfileSearch(GPConnection* connection, const char nick[GP_NICK_LEN],
    const char uniquenick[GP_UNIQUENICK_LEN], const char email[GP_EMAIL_LEN], const char firstname[GP_FIRSTNAME_LEN],
    const char lastname[GP_LASTNAME_LEN], int icquin, int skip, GPEnum blocking, GPCallback callback, void* param)
{
    GPISearchData* data;
...
GPResult gpiProfileSearchUniquenick(GPConnection* connection, const char uniquenick[GP_UNIQUENICK_LEN],
    const int namespaceIDs[], int numNamespaces, GPEnum blocking, GPCallback callback, void* param)
{
    GPISearchData* data;
...
GPResult gpiGetUserNicks(GPConnection* connection, const char email[GP_EMAIL_LEN], const char password[GP_PASSWORD_LEN],
...
GPResult gpiCheckUser(GPConnection* connection, const char nick[GP_NICK_LEN], const char email[GP_EMAIL_LEN],
...
GPResult gpiNewUser(GPConnection* connection, const char nick[GP_NICK_LEN], const char uniquenick[GP_UNIQUENICK_LEN],
...
GPResult gpiOthersBuddyList(
    GPConnection* connection, int* profiles, int numOfProfiles, GPEnum blocking, GPCallback callback, void* param)
{
    GPISearchData* data;
    ...
    do
    {
        ...
        if (operation->state == GPI_CONNECTING)
        {
            ...
            if (state == GPI_DISCONNECTED)
                CallbackError(connection, GP_SERVER_ERROR, GP_SEARCH_CONNECTION_FAILED,
        ...
        else if (operation->state == GPI_WAITING)
        {
            ...
            if (result != GP_NO_ERROR)
            {
                if (result == GP_NETWORK_ERROR)
                    CallbackError(connection, GP_NETWORK_ERROR, GP_SEARCH_CONNECTION_FAILED,
            ...
            if (strstr(data->inputBuffer.buffer, "\\final\\") != NULL)
            {
                ...
                if ((data->type == GPI_SEARCH_PROFILE) || (data->type == GPI_SEARCH_PROFILE_UNIQUENICK))
                {
                    ...
                    if ((more == GP_MORE) && (arg.more == GP_MORE))
                        CHECK_RESULT(gpiProfileSearch(connection, data->nick, data->uniquenick, data->email,
                            data->firstname, data->lastname, data->icquin, arg.numMatches + data->skip,
                            (GPEnum)operation->blocking, operation->callback.callback, operation->callback.param));

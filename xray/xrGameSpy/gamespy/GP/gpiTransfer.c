...
static GPResult gpiNewReceiverTransfer(
    GPConnection* connection, GPITransfer** transfer, GPProfile profile, GPITransferID* transferID)
{
#ifdef WIN32
...
void gpiCancelTransfer(GPConnection* connection, GPITransfer* transfer)
{
    ...
    if (transfer->peer)
    {
        // Start the message.
        /////////////////////
        if (gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_TRANSFER_CANCEL,
                (GPITransferID_st)&transfer->transferID) == GP_NO_ERROR)
        {
            // Finish the message.
            //////////////////////
...
void gpiSkipFile(GPConnection* connection, GPITransfer* transfer, int file, int reason)
{
    ...
    if (gpiPeerStartTransferMessage(
            connection, transfer->peer, GP_FILE_SKIP, (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
        return;
...
static GPIBool gpiHandleSendRequest(GPConnection* connection, GPIPeer* peer, GPITransferID* transferID,
    const char* headers, const char* buffer, int bufferLen)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPITransfer* transfer;
    GPIFile* file;
...
static GPIBool gpiHandleSendReply(
    GPConnection* connection, GPITransfer* transfer, const char* headers, const char* buffer, int bufferLen)
{
    GPIConnection* iconnection = (GPIConnection*)*connection;
    GPTransferCallbackArg* arg;
    char intValue[16];
...
static GPIBool gpiHandleEnd(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    else
    {
        ...
        ///////////////////////
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_END, (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
            return GPIFalse;
        gpiFinishTransferMessage(connection, transfer, NULL, 0);
#endif
    }
    ...
    GPConnection* connection, GPITransfer* transfer, const char* headers, const char* buffer, int bufferLen)
{
    ...
    else
    {
        ...
        //////////////////////////
        if (gpiPeerStartTransferMessage(
                connection, transfer->peer, GPI_BM_FILE_DATA, (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
            return GPIFalse;
...
static GPIBool gpiHandleTransferThrottle(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    if (transfer->sender)
    {
        if (gpiPeerStartTransferMessage(connection, transfer->peer, GPI_BM_FILE_TRANSFER_THROTTLE,
                (GPITransferID_st)&transfer->transferID) != GP_NO_ERROR)
            return GPIFalse;
            ...
            if (!file->file)
            {
                ...
                if (file->file)
                {
                    ...
                    if (arg)
                    {
                        ...
                        arg->index = transfer->currentFile;
                        arg->type = GP_FILE_BEGIN;
                        gpiAddCallback(connection, iconnection->callbacks[GPI_TRANSFER_CALLBACK], arg, NULL,
...
GPResult gpiSendTransferReply(
    GPConnection* connection, const GPITransferID* transferID, GPIPeer* peer, int result, const char* msg)
{
    char buffer[32];
...
void gpiHandleTransferMessage(
    GPConnection* connection, GPIPeer* peer, int type, const char* headers, const char* buffer, int len)
{
    char value[64];
    ...
    switch (type)
    {
    case GPI_BM_FILE_SEND_REPLY: success = gpiHandleSendReply(connection, transfer, headers, buffer, len); break;
    case GPI_BM_FILE_BEGIN: success = gpiHandleBegin(connection, transfer, headers); break;
    case GPI_BM_FILE_END: success = gpiHandleEnd(connection, transfer, headers); break;
    case GPI_BM_FILE_DATA: success = gpiHandleData(connection, transfer, headers, buffer, len); break;
    case GPI_BM_FILE_SKIP: success = gpiHandleSkip(connection, transfer, headers); break;
    case GPI_BM_FILE_TRANSFER_THROTTLE: success = gpiHandleTransferThrottle(connection, transfer, headers); break;
    case GPI_BM_FILE_TRANSFER_CANCEL: success = gpiHandleTransferCancel(connection, transfer); break;
    case GPI_BM_FILE_TRANSFER_KEEPALIVE: success = gpiHandleTransferKeepalive(connection, transfer); break;
    default: success = GPITrue;
    }

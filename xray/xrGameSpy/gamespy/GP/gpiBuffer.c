...
GPResult gpiAppendStringToBufferLen(
    GPConnection* connection, GPIBuffer* outputBuffer, const char* string, int stringLen)
{
    int len;
    int size;
    char* output;
...
static GPResult gpiSendData(
    GPConnection* connection, SOCKET sock, const char* buffer, int bufferLen, GPIBool* closed, int* sent, char id[3])
{
    int rcode;
    ...
    else
    {
        ...
        {
            static int sendCount;
            gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "SENT%04d(%s): %d\n",
                sendCount++, id, rcode);
        }
#endif
...
GPResult gpiRecvToBuffer(
    GPConnection* connection, SOCKET sock, GPIBuffer* inputBuffer, int* bytesRead, GPIBool* connClosed, char id[3])
{
    char* buffer;
    int len;
    int size;
    ...
    do
    {
        ...
        else
        {
#if defined(GPI_DUMP_NET_TRAFFIC) && defined(GSI_COMMON_DEBUG)
            {
                ...
                buf[rcode] = '\0';
                gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "RECV%04d(%s): %s\n",
                    recvCount++, id, buf);
                freeclear(buf);
            }
#elif defined(GSI_COMMON_DEBUG)
            {
                static int recvCount;
                gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Network, GSIDebugLevel_RawDump, "RECV%04d(%s): %d\n",
                    recvCount++, id, rcode);
            }
#endif
...
GPResult gpiSendFromBuffer(GPConnection* connection, SOCKET sock, GPIBuffer* outputBuffer, GPIBool* connClosed,
    GPIBool clipSentData, char id[3])
{
    GPIBool closed;
    int sent;
    int total;
...
GPResult gpiSendBufferToPeer(GPConnection* connection, unsigned int ip, unsigned short port, GPIBuffer* outputBuffer,
...
GPResult gpiReadMessageFromBuffer(
    GPConnection* connection, GPIBuffer* inputBuffer, char** message, int* type, int* plen)
{
    char* str;
    int len;
    char intValue[16];

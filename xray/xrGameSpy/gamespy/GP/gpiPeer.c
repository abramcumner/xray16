    ...
    do
    {
        ...
        if (buffer != NULL)
        {
            ...
            case GPI_BM_PONG:
                ...
                break;
#endif
            case GPI_BM_KEYS_REQUEST: CHECK_RESULT(gpiBuddyHandleKeyRequest(connection, peer)); break;
    ...
    for (peer = iconnection->peerList; peer != NULL; peer = nextPeer)
    {
        ...
        else
        {
            ...
            if ((peer->state == GPI_PEER_DISCONNECTED) || (result != GP_NO_ERROR) || (time(NULL) > peer->timeout))
            {
                // Remove it.
                /////////////
                gsDebugFormat(GSIDebugCat_GP, GSIDebugType_Misc, GSIDebugLevel_Notice, "Peer disconnected, pid: %d",
                    peer->profile);
                gpiRemovePeer(connection, peer);
            }
        }
    ...
    if (profile->buddyStatusInfo)
    {
        ...
        if (aPeerState != GS_UDP_PEER_CONNECTED || aPeerState != GS_UDP_PEER_CONNECTING)
        {
            anError = gsUdpEngineStartTalkingToPeer(profile->buddyStatusInfo->buddyIp,
                profile->buddyStatusInfo->buddyPort, iconnection->mHeader, GPI_PEER_TIMEOUT);
...
GPResult gpiPeerStartTransferMessage(
    GPConnection* connection, GPIPeer* peer, int type, const struct GPITransferID_s* transferID)
{
    char buffer[64];
    GPITransferID tid;
...
void gpiPeerMessageCallback(unsigned int ip, unsigned short port, unsigned char* message, unsigned int messageLength,
...
void gpiPeerAcceptedCallback(
    unsigned int ip, unsigned short port, GSUdpErrorCode error, gsi_bool rejected, void* userData)
{
    GPConnection* connection = (GPConnection*)userData;
    GPIPeer* aPeer;
    IN_ADDR anAddr;

...
GPResult gpiSendBuddyMessage(
    GPConnection* connection, int profileid, int type, const char* message, int sendOption, GPIPeerOp* peerOp)
{
    GPIPeer* peer;
    GPIProfile* profile;
    // GPIConnection *iconnection = (GPIConnection *)*connection;
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; i < numKeys; i++)
            {
                ...
                if (gpiStatusInfoCheckKey(
                        connection, pProfile->buddyStatusInfo->extendedInfoKeys, decodeKey, &checkKey) == GP_NO_ERROR &&
                    checkKey == NULL)
                {

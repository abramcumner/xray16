...
static int CheckMagic(char* data) { return (memcmp(data, NNMagicData, NATNEG_MAGIC_LEN) == 0); }
...
static void SendInitPackets(NATNegotiator neg)
{
    ...
    if (p->Packet.Init.usegameport && !neg->initAckRecv[NN_PT_GP])
    {
        ...
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Sending INIT (GP) to %s:%d...\n",
            inet_ntoa(*(struct in_addr*)&matchup1ip), MATCHUP_PORT1);
...
static void SendPingPacket(NATNegotiator neg)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
        "Sending PING to %s:%d (got remote data: %d)\n", inet_ntoa(*(struct in_addr*)&neg->guessedIP), neg->guessedPort,
        neg->gotRemoteData);
    SendPacket((neg->gameSock != INVALID_SOCKET) ? neg->gameSock : neg->negotiateSock, neg->guessedIP, neg->guessedPort,
        &p, CONNECTPACKET_SIZE);
...
NegotiateError NNBeginNegotiation(int cookie, int clientindex, NegotiateProgressFunc progresscallback,
...
NegotiateError NNBeginNegotiationWithSocket(SOCKET gamesocket, int cookie, int clientindex,
    NegotiateProgressFunc progresscallback, NegotiateCompletedFunc completedcallback, void* userdata)
{
    NATNegotiator neg;
...
#if defined(GSI_COMMON_DEBUG)
    {
        ...
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Negotiate Socket: %d\n",
...
static void NegotiateThink(NATNegotiator neg)
{
    ...
    if (neg->negotiateSock != INVALID_SOCKET)
    {
        ...
        while (CanReceiveOnSocket(neg->negotiateSock))
        {
            ...
            {
                gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "RECV SOCKET ERROR: %d\n",
...
static void ProcessConnectPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
        ...
        inet_ntoa(*(struct in_addr*)&p->Packet.Connect.remoteIP), ntohs(p->Packet.Connect.remotePort));
    // send an ack..
...
static void ProcessPingPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice,
        ...
        ntohs(fromaddr->sin_port), p->Packet.Connect.gotyourdata, p->Packet.Connect.finished);
...
static void ProcessInitPacket(NATNegotiator neg, NatNegPacket* p, struct sockaddr_in* fromaddr)
{
    ...
    case NN_INITACK:
        ...
        if (p->Packet.Init.porttype > NN_PT_NN3)
            return; // invalid port
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Notice, "Got init ack for port %d\n",

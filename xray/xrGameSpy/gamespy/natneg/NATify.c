...
int DiscoverReachability(SOCKET sock, unsigned int ip, unsigned short port, int portType)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "(%d) Sending ERT Request to %s\n",
...
int DiscoverMapping(SOCKET sock, unsigned int ip, unsigned short port, int portType, int id)
{
    ...
    gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "Sending ADDRESS CHECK %d to %s\n",
...
void OutputMapping(const AddressMapping* theMap)
{
    ...
    {
        gsDebugFormat(GSIDebugCat_NatNeg, GSIDebugType_Network, GSIDebugLevel_Comment, "Discovered map: [%s] -> [%s]\n",
    ...
    if (sock != INVALID_SOCKET)
    {
        ...
        while (CanReceiveOnSocket(sock))
        {
            ...
            else if (ptype == NN_ADDRESS_REPLY)
            {
                ...
                switch (p.cookie)
                {
                case packet_map1a: gotADDRESS1a = gsi_true; break;
                case packet_map1b: gotADDRESS1b = gsi_true; break;
                case packet_map2: gotADDRESS2 = gsi_true; break;
                case packet_map3: gotADDRESS3 = gsi_true; break;
                }
...
int NatifyThink(SOCKET sock, NAT* nat) { return (Think(sock, nat)); }

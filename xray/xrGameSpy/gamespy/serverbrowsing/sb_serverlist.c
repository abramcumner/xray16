...
static int prevKeyCompare(SBServer server1, SBServer server2)
{
    ...
    case sbcm_strcase:
        diff = strcmp(
            SBServerGetStringValueA(server1, prevsortkey, ""), SBServerGetStringValueA(server2, prevsortkey, ""));
        break;
    case sbcm_stricase:
        diff = strcasecmp(
            SBServerGetStringValueA(server1, prevsortkey, ""), SBServerGetStringValueA(server2, prevsortkey, ""));
...
static int GS_STATIC_CALLBACK StrNoCaseKeyCompare(const void* entry1, const void* entry2)
{
    ...
    diff = strcasecmp(
        SBServerGetStringValueA(server1, currsortkey, ""), SBServerGetStringValueA(server2, currsortkey, ""));
...
void SBServerListSort(SBServerList* slist, SBBool ascending, SortInfo sortinfo)
{
    ...
    {
    case sbcm_int: comparator = IntKeyCompare; break;
    case sbcm_float: comparator = FloatKeyCompare; break;
    case sbcm_strcase: comparator = StrCaseKeyCompare; break;
    case sbcm_stricase: comparator = StrNoCaseKeyCompare; break;
    default: comparator = StrNoCaseKeyCompare;
    }
...
int SBServerListCount(SBServerList* slist) { return ArrayLength(slist->servers); }

SBServer SBServerListNth(SBServerList* slist, int i) { return *(SBServer*)ArrayNth(slist->servers, i); }
...
void SBServerListInit(SBServerList* slist, const char* queryForGamename, const char* queryFromGamename,
    const char* queryFromKey, int queryFromVersion, SBBool lanBrowse, SBListCallBackFn callback, void* instance)
{
    assert(slist != NULL);
...
SBError SBServerListConnectAndQuery(
    SBServerList* slist, const char* fieldList, const char* serverFilter, int options, int maxServers)
{
    SBError err;
    int ret;
    int requestLen;
...
static int AllKeysPresent(SBServerList* slist, char* buf, int len)
{
    ...
    for (i = 0; i < numkeys; i++)
    {
        ...
        case KEYTYPE_STRING:
            ...
            break;
        default: assert(0); return 0; // error - unknown key type
        }
        if (len < 0)
            return 0; // not enough..
    }
...
#ifdef GSI_UNICODE
const unsigned short* SBLastListErrorW(SBServerList* slist) { return slist->lasterror_W; }
#endif
...
static SBError ProcessAdHocData(SBServerList* slist)
{
    ...
    while (
        slist->inbufferlen >= 3) // while there is at least 1 message in there.. (2 bytes for the size + 1 for the type)
    {
        ...
        {
        case PUSH_KEYS_MESSAGE: ret = ProcessPushKeyList(slist, slist->inbuffer + 3, msglen - 3); break;
        case PUSH_SERVER_MESSAGE: ret = ProcessPushServer(slist, slist->inbuffer + 3, msglen - 3); break;
        case KEEPALIVE_MESSAGE: // just need to send it back..
            i = (int)send(slist->slsocket, slist->inbuffer, msglen, 0);
            if (i <= 0)
                return sbe_connecterror;
            break;
        case DELETE_SERVER_MESSAGE: ret = ProcessDeleteServer(slist, slist->inbuffer + 3, msglen - 3); break;
        case MAPLOOP_MESSAGE: ret = ProcessMaploop(slist, slist->inbuffer + 3, msglen - 3); break;
        case PLAYERSEARCH_MESSAGE: ret = ProcessPlayerSearch(slist, slist->inbuffer + 3, msglen - 3); break;
        }
...
SBError SBSendPlayerSearchRequest(
    SBServerList* slist, char* searchName, int searchOptions, int maxResults, SBPlayerSearchCallbackFn callback)
{

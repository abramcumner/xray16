...
static void ListCallback(SBServerList* serverlist, SBListCallbackReason reason, SBServer server, void* instance)
{
    ...
    case slc_initiallistcomplete:
        ...
        break;
    case slc_disconnected: break;
    case slc_queryerror: sb->BrowserCallback(sb, sbc_queryerror, NULL, sb->instance); break;
    case slc_publicipdetermined: SBQueryEngineSetPublicIP(&sb->engine, sb->list.mypublicip); break;
    case slc_serverchallengereceived: break;
    }
...
static void EngineCallback(SBQueryEnginePtr engine, SBQueryEngineCallbackReason reason, SBServer server, void* instance)
{
    ...
    switch (reason)
    {
    case qe_updatefailed: sb->BrowserCallback(sb, sbc_serverupdatefailed, server, sb->instance); break;
    case qe_updatesuccess: sb->BrowserCallback(sb, sbc_serverupdated, server, sb->instance); break;
    case qe_engineidle: sb->BrowserCallback(sb, sbc_updatecomplete, server, sb->instance); break;
...
ServerBrowser ServerBrowserNewA(const char* queryForGamename, const char* queryFromGamename, const char* queryFromKey,
    int queryFromVersion, int maxConcUpdates, int queryVersion, SBBool lanBrowse, ServerBrowserCallback callback,
    void* instance)
{
    ServerBrowser sb;
...
ServerBrowser ServerBrowserNewW(const unsigned short* queryForGamename, const unsigned short* queryFromGamename,
    const unsigned short* queryFromKey, int queryFromVersion, int maxConcUpdates, int queryVersion, SBBool lanBrowse,
...
SBError ServerBrowserLimitUpdateA(ServerBrowser sb, SBBool async, SBBool disconnectOnComplete,
    const unsigned char* basicFields, int numBasicFields, const char* serverFilter, int maxServers)
{
...
SBError ServerBrowserLimitUpdateW(ServerBrowser sb, SBBool async, SBBool disconnectOnComplete,
    const unsigned char* basicFields, int numBasicFields, const unsigned short* serverFilter, int maxServers)
{
    char serverFilter_A[1024];
    if (serverFilter != NULL)
        UCS2ToUTF8String(serverFilter, serverFilter_A);
...
SBError ServerBrowserSendMessageToServerA(
    ServerBrowser sb, const char* ip, unsigned short port, const char* data, int len)
{
    return SBSendMessageToServer(&sb->list, inet_addr(ip), htons(port), data, len);
}
#ifdef GSI_UNICODE
SBError ServerBrowserSendMessageToServerW(
    ServerBrowser sb, const unsigned short* ip, unsigned short port, const char* data, int len)
{
    char ip_A[128];
...
#ifdef GSI_UNICODE
SBError ServerBrowserSendNatNegotiateCookieToServerW(
    ServerBrowser sb, const unsigned short* ip, unsigned short port, int cookie)
{
    char ip_A[128];
    UCS2ToAsciiString(ip, ip_A);
...
static void NatNegCompletedCallback(
    NegotiateResult result, SOCKET gamesocket, struct sockaddr_in* remoteaddr, void* userdata)
{
...
SBError ServerBrowserAuxUpdateIPA(
    ServerBrowser sb, const char* ip, unsigned short port, SBBool viaMaster, SBBool async, SBBool fullUpdate)
{
    SBError err = sbe_noerror;
...
#ifdef GSI_UNICODE
SBError ServerBrowserAuxUpdateIPW(
    ServerBrowser sb, const unsigned short* ip, unsigned short port, SBBool viaMaster, SBBool async, SBBool fullUpdate)
{
    char ip_A[128];
    UCS2ToAsciiString(ip, ip_A);
...
const unsigned short* ServerBrowserErrorDescW(ServerBrowser sb, SBError error)
{
    switch (error)
    {
    case sbe_noerror: return L"None"; break;
    case sbe_socketerror: return L"Socket creation error"; break;
    case sbe_dnserror: return L"DNS lookup error"; break;
    case sbe_connecterror: return L"Connection failed"; break;
    case sbe_dataerror: return L"Data stream error"; break;
    case sbe_allocerror: return L"Memory allocation error"; break;
    case sbe_paramerror: return L"Function parameter error"; break;
    case sbe_duplicateupdateerror: return L"Duplicate update request error"; break;
    }
    return L"";
...
const char* ServerBrowserListQueryErrorA(ServerBrowser sb) { return SBLastListErrorA(&sb->list); }
#ifdef GSI_UNICODE
const unsigned short* ServerBrowserListQueryErrorW(ServerBrowser sb) { return SBLastListErrorW(&sb->list); }
#endif
...
SBServer ServerBrowserGetServer(ServerBrowser sb, int index) { return SBServerListNth(&sb->list, index); }
...
int ServerBrowserCount(ServerBrowser sb) { return SBServerListCount(&sb->list); }
...
char* ServerBrowserGetMyPublicIP(ServerBrowser sb) { return (char*)inet_ntoa(*(struct in_addr*)&sb->list.mypublicip); }

unsigned int ServerBrowserGetMyPublicIPAddr(ServerBrowser sb) { return sb->list.mypublicip; }

void ServerBrowserDisconnect(ServerBrowser sb) { SBServerListDisconnect(&sb->list); }

// Allows the user to specify a broadcast address for LAN hosting
void ServerBrowserLANSetLocalAddr(ServerBrowser sb, const char* theAddr) { sb->list.mLanAdapterOverride = theAddr; }

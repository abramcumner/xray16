...
GHIEncryptionResult ghiEncryptorSslEncryptFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor,
    const char* thePlainTextBuffer, int thePlainTextLength, char* theEncryptedBuffer, int* theEncryptedLength)
{
    int encodeResult = 0;
...
GHIEncryptionResult ghiEncryptorSslDecryptFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor,
    const char* theEncryptedBuffer, int* theEncryptedLength, char* theDecryptedBuffer, int* theDecryptedLength)
{
...
GHIEncryptionResult ghiEncryptorSslEncryptSend(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor,
    const char* thePlainTextBuffer, int thePlainTextLength, int* theBytesSentOut)
{
    gsRevoExInterface* sslInterface = (gsRevoExInterface*)theEncryptor->mInterface;
    int result = 0;
...
GHIEncryptionResult ghiEncryptorSslDecryptRecv(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor,
...
static GHIEncryptionResult ghiEncryptorProcessSSLHandshake(
    struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data);
...
GHIEncryptionResult ghiEncryptorSslStartFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    // ghttpEncryptorSetNBOBytesFromHBOInt(helloMsg.time, (gsi_u32)current_time(), 4); // 4 byte time value (for
    // randomness)
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
    ...
    MD5Update(&sslInterface->finishHashMD5, (unsigned char*)&helloMsg + sizeof(gsSSLRecordHeaderMsg),
        sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg));
    SHA1Input(&sslInterface->finishHashSHA1, (unsigned char*)&helloMsg + sizeof(gsSSLRecordHeaderMsg),
        sizeof(gsSSLClientHelloMsg) - sizeof(gsSSLRecordHeaderMsg));
    ...
    const char* thePlainTextBuffer, int thePlainTextLength, char* theEncryptedBuffer, int* theEncryptedLength)
{
    if (theEncryptor != NULL)
    {
        ...
        else
        {
            ...
            RC4Encrypt(&sslInterface->sendRC4, (const unsigned char*)thePlainTextBuffer,
    ...
    const char* theEncryptedBuffer, int* theEncryptedLength, char* theDecryptedBuffer, int* theDecryptedLength)
{
    ...
    while (readPos < *theEncryptedLength)
    {
        ...
        case GS_SSL_CONTENT_HANDSHAKE: {
            ...
            readPos += sizeof(gsSSLRecordHeaderMsg);
            if (connection->encryptor.mSessionEstablished)
                RC4Encrypt(&sslInterface->recvRC4, (const unsigned char*)&theEncryptedBuffer[readPos],
        ...
        default: readPos += sizeof(gsSSLRecordHeaderMsg); return GHIEncryptionResult_Error; // unhandled content type
        };
...
//    for tag specifics.
GHIEncryptionResult ghiEncryptorProcessSSLHandshake(
    struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data)
{
    ...
    while (data->pos < data->len)
    {
            ...
            while (data->pos < certListEndPos)
            {
                ...
                if (certCount == 1)
                {
                    ...
                    if (temp - 1 > GS_LARGEINT_BINARY_SIZE / sizeof(char))
                        return GHIEncryptionResult_Error;
                    sslInterface->serverpub.modulus.mLength = (unsigned int)((temp - 1) /
        ...
        else if (messageType == GS_SSL_HANDSHAKE_SERVERHELLODONE)
        {
            ...
            // add it to the running handshake hash
            MD5Update(&sslInterface->finishHashMD5, (unsigned char*)&data->data[messageStart],
                (unsigned int)(data->pos - messageStart));
            SHA1Input(&sslInterface->finishHashSHA1, (unsigned char*)&data->data[messageStart],
                (unsigned int)(data->pos - messageStart));
...
#pragma message("!!!WARNING: SSL Random disable for debug purposes.  SSL not secured!!!")
            ...
            ghiEncryptorWriteNBOLength(clientKeyExchange->header.lengthNBO,
                (int)(sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES + 4), 2);
            clientKeyExchange->handshakeType = GS_SSL_HANDSHAKE_CLIENTKEYEXCHANGE;
            ghiEncryptorWriteNBOLength(clientKeyExchange->lengthNBO,
                (int)(sslInterface->serverpub.modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES), 3);
            //    encrypt the preMasterSecret using the server's public key (store result in sendbuffer)
            gsCryptRSAEncryptBuffer(&sslInterface->serverpub, sslInterface->premastersecret, GS_SSL_MASTERSECRET_LEN,
            ...
            // output the message MAC  (hash(MAC_write_secret+pad_2+
            // hash(MAC_write_secret+pad_1+seq_num+length+content))); Re-using the finishHashMD5 since it has already
            // been allocated
            MD5Init(&sslInterface->finishHashMD5);
            MD5Update(&sslInterface->finishHashMD5, (unsigned char*)sslInterface->clientWriteMACSecret,
            ...
            RC4Encrypt(&sslInterface->sendRC4, ((unsigned char*)finalHandshake) + sizeof(gsSSLRecordHeaderMsg),
                ((unsigned char*)finalHandshake) + sizeof(gsSSLRecordHeaderMsg), 56);
        }
